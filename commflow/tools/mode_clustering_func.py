...
def pathways_overview(adata, human=True, moments=False):
    ...
    '''
    # if human:
    #     lig = loadDB('/Users/federicobocci/Desktop/ccc_project/pathway_database/ligand_human.csv')
    #     rec = loadDB('/Users/federicobocci/Desktop/ccc_project/pathway_database/receptor_human.csv')
    # else:
    #     lig = loadDB('/Users/federicobocci/Desktop/ccc_project/pathway_database/ligand_mouse.csv')
    #     rec = loadDB('/Users/federicobocci/Desktop/ccc_project/pathway_database/receptor_mouse.csv')
    if human:
        lig = loadDB('/Users/federicobocci/Desktop/commflow_project/ccc_project/pathway_database/ligand_human.csv')
        rec = loadDB('/Users/federicobocci/Desktop/commflow_project/ccc_project/pathway_database/receptor_human.csv')
    else:
    ...
    for p in path_list:
        ...
        if len(rec_list)==0 or len(lig_list)==0:
            ...
            rec_common, lig_common = find_genes(adata, p, human=human, save_genes=False, print_info=False, return_lists=True)
...
def single_path_sim(adata,
    ...
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #
    # possible improvement: cluster using all receptors/ligands/targets instead of averaging
    #
    # do not take average and instead append the count matrices for L, R, T
    #
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    # if 'similarity' not in adata.uns.keys():
    #     adata.uns['similarity'] = {}
    # adata.uns['similarity'][pathway] = sim_mat
    print('finished sim_mat for ' + pathway + ', starting spectral analysis')

    # spectral_analysis(adata, sim_mat, pathway, print_info=print_info)
    # print('fnished spectral analysis')
...
def all_path_sim(adata, annotations, nsim=5, kmin=3, kmax=6, target=True, human=True, moments=False,
                 neighbor_average=False, method='expression', n=10, top_target=10, order=None, seed=100, print_info=False):
    ...
    '''
    select_pathways(adata, human=human, method=method, n=n)
    ...
    for p in sel_path:
        ...
        spectral_analysis(adata, sim_mat, p, print_info=print_info)
        print('fnished spectral analysis')
