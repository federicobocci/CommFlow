...
def pattern_marker(ax, adata, pattern, ntop=25, human=True, cmap=plt.cm.tab20,
    ...
    # if human:
    #     lig = loadDB('/Users/federicobocci/Desktop/ccc_EMT/pathway_database/ligand_human.csv')
    #     rec = loadDB('/Users/federicobocci/Desktop/ccc_EMT/pathway_database/receptor_human.csv')
    # else:
    #     lig = loadDB('/Users/federicobocci/Desktop/ccc_EMT/pathway_database/ligand_mouse.csv')
    #     rec = loadDB('/Users/federicobocci/Desktop/ccc_EMT/pathway_database/receptor_mouse.csv')
    if human:
...
def pattern(adata, p, key='cluster', order=None, human=True, ntop=25, marker_cmap= plt.cm.tab20,                       # general function parameters
            ...
            savefig=True, figname='pattern_detail.pdf', format='pdf', dpi=300       # figure parameters
            ):
...
def pattern_summary(adata, key, order=None, panel_per_row=4, panel_length=3, panel_height=3, cmap=plt.cm.tab20,
                    fontsize=10,
                    savefig=True, figname='pattern_summary.pdf', format='pdf', dpi=300):
    if order:
        states = order
    else:
